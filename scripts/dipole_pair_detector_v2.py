...
TOP_CROP_COUNT = 50
CROP_HALF_SIZE = 42
REVIEW_DIR = Path("docs/review_runs/dipole_pair_detector_v2")
...
def match_pairs(positive_blobs: Sequence[Blob], negative_blobs: Sequence[Blob], target_vector: Tuple[float, float]) -> List[PairMatch]:
    ...
    for pos in positive_blobs:
        for neg in negative_blobs:
            ...
            confidence_category = classify_pair(
                vector_error,
                amplitude_balance,
                pos_peak_z,
                neg_peak_abs_z,
    ...
    matches.sort(key=lambda item: item.score_v2, reverse=True)
...
def save_pair_crop(image: np.ndarray, z: np.ndarray, pair: PairMatch, path: Path) -> None:
    ...
    for ax in axes:
        ax.set_axis_off()
    fig.suptitle("pair {0:04d} {1} score={2:.2f}".format(pair.pair_id, pair.confidence_category, pair.score_v2), fontsize=9)
    fig.tight_layout()
    fig.savefig(path)
    plt.close(fig)
...
def make_contact_sheet_from_crops(crop_paths: Sequence[Path], matches: Sequence[PairMatch], contact_path: Path, empty_message: str) -> None:
    ...
    for ax, thumb, pair in zip(axes_array, thumb_images, matches):
        ax.imshow(thumb)
        ax.set_title("#{0} {1} s={2:.1f}".format(pair.pair_id, pair.confidence_category.replace("_confidence", ""), pair.score_v2), fontsize=8)
        ax.set_axis_off()
...
def create_confidence_contact_sheet(
    ...
    category_matches = [match for match in matches if match.confidence_category == confidence_category][:TOP_CROP_COUNT]
    crop_dir_name = "crops_{0}".format(confidence_category.replace("_confidence", ""))
    crops_dir = out_dir / crop_dir_name
    ensure_dir(crops_dir)
    for old_crop in crops_dir.glob("pair_*.png"):
        old_crop.unlink()
...
def build_review_package(
    ...
    pairs_filename = "pairs_{0}_v2.csv".format(mode)
    overlay_filename = "pair_overlay_{0}_v2.png".format(mode)
    contact_filename = "contact_sheet_{0}_v2.png".format(mode)
    ...
    if mode == "manual":
        ...
        copy_image_as_jpg(out_dir / overlay_filename, review_dir / "pair_overlay_manual_v2.jpg")
        copy_image_as_jpg(out_dir / contact_filename, review_dir / "contact_sheet_manual_v2.jpg")
        copy_image_as_jpg(out_dir / "high_confidence_contact_sheet.png", review_dir / "high_confidence_contact_sheet.jpg")
        copy_image_as_jpg(out_dir / "medium_confidence_contact_sheet.png", review_dir / "medium_confidence_contact_sheet.jpg")
        crop_source = out_dir / "crops_manual_v2"
        crop_destination = review_dir / "crops_manual_v2"
        high_source = out_dir / "crops_high"
        medium_source = out_dir / "crops_medium"
    else:
        ...
        copy_image_as_jpg(out_dir / overlay_filename, review_dir / "pair_overlay_auto_v2.jpg")
        copy_image_as_jpg(out_dir / contact_filename, review_dir / "contact_sheet_auto_v2.jpg")
        crop_source = out_dir / "crops_auto_v2"
        crop_destination = review_dir / "crops_auto_v2"
        high_source = None
        medium_source = None
...
def update_review_readme(review_dir: Path) -> None:
    ...
    v1_manual = load_json_if_exists(Path("docs/review_runs/dipole_pair_detector_v1/summary_manual.json"))
    v1_auto = load_json_if_exists(Path("docs/review_runs/dipole_pair_detector_v1/summary_auto.json"))
    ...
    lines = [
        ...
        "| Metric | v1 | v2 |",
        "| --- | ---: | ---: |",
        "| Manual matched count | {0} | {1} |".format(
            v1_manual.get("matched_pair_count", "n/a") if v1_manual else "n/a",
            manual.get("matched_pair_count", "n/a") if manual else "n/a",
        ),
        "| Auto matched count | {0} | {1} |".format(
            v1_auto.get("matched_pair_count", "n/a") if v1_auto else "n/a",
            auto.get("matched_pair_count", "n/a") if auto else "n/a",
        ),
        "| Manual target dx,dy | {0} | {1} |".format(
            "({0:.3f},{1:.3f})".format(v1_manual["target_vector"]["dx"], v1_manual["target_vector"]["dy"]) if v1_manual else "n/a",
            "({0:.3f},{1:.3f})".format(manual["target_vector"]["dx"], manual["target_vector"]["dy"]) if manual else "n/a",
        ),
        "| Auto target dx,dy | {0} | {1} |".format(
            "({0:.3f},{1:.3f})".format(v1_auto["target_vector"]["dx"], v1_auto["target_vector"]["dy"]) if v1_auto else "n/a",
            "({0:.3f},{1:.3f})".format(auto["target_vector"]["dx"], auto["target_vector"]["dy"]) if auto else "n/a",
        ),
        "",
        "## V2 Confidence Counts",
        "",
        "- Manual: `{0}`".format(category_summary_text(manual)),
        "- Auto: `{0}`".format(category_summary_text(auto)),
        "",
        "## Seed Pair Rank Check",
        "",
    ]
    if manual and manual.get("seed_pair_ranks"):
        for item in manual["seed_pair_ranks"]:
            lines.append(
        ...
        lines.append("- Not available until manual mode is run.")
    lines.extend(["", "## Auto Vector Confidence", ""])
    if auto and auto.get("auto_vector_summary"):
        summary = auto["auto_vector_summary"]
        v1_conf = None
        if v1_auto and v1_auto.get("auto_vector_summary"):
            v1_conf = v1_auto["auto_vector_summary"].get("confidence")
        lines.append("- v1 confidence: `{0}`".format(v1_conf if v1_conf is not None else "n/a"))
        lines.append("- v2 confidence: `{0}`".format(summary.get("confidence")))
        lines.append("- Improved: `{0}`".format(bool(v1_conf is not None and summary.get("confidence", 0) > v1_conf)))
        warning = summary.get("confidence_warning")
        if warning:
            lines.append("- Warning: {0}".format(warning))
    else:
        lines.append("- Not available until auto-vector mode is run.")
...
def confidence_counts(matches: Sequence[PairMatch]) -> Dict[str, int]:
    ...
    for match in matches:
        counts[match.confidence_category] = counts.get(match.confidence_category, 0) + 1
    return counts
...
def run_detector(args: argparse.Namespace) -> Dict:
    ...
    plot_pair_overlay(z, matches, target_vector, out_dir / overlay_filename, "{0} v2".format(mode))
    plot_pair_overlay(z, matches, target_vector, out_dir / "pair_overlay.png", "{0} v2".format(mode))
    create_crops_and_contact_sheet(image, z, matches, out_dir, contact_filename, "crops_{0}_v2".format(mode))
    if mode == "manual":
        create_confidence_contact_sheet(image, z, matches, out_dir, "high_confidence", "high_confidence_contact_sheet.png")
        create_confidence_contact_sheet(image, z, matches, out_dir, "medium_confidence", "medium_confidence_contact_sheet.png")
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Dipole Pair Detector v2")
