...
TEMPLATE_SIGMA = 4.0
TEMPLATE_PATCH_RADIUS = 48
ISOLATION_CENTER_RADIUS = 5.0
ISOLATION_RING_MARGIN = 4.0
ISOLATION_RING_WIDTH = 14.0
TOP_CROP_COUNT = 50
CROP_HALF_SIZE = 42
REVIEW_DIR = Path("docs/review_runs/dipole_pair_detector_v3")
...
class PairMatch:
    ...
    vector_error: float
    pos_peak_z: float
    neg_peak_abs_z: float
    amplitude_balance: float
    area_ratio: float
    shape_penalty: float
    pos_num_possible_partners: int
    neg_num_possible_partners: int
    num_possible_partners_pos: int
    num_possible_partners_neg: int
    num_possible_partners: int
    aspect_ratio_pos: float
    aspect_ratio_neg: float
    local_contrast_score: float
    template_score: float
    isolation_score: float
    isolation_score_clipped: float
    size_mismatch_penalty: float
    partner_ambiguity_penalty: float
...
def compute_possible_partner_counts(
    positive_blobs: Sequence[Blob],
    negative_blobs: Sequence[Blob],
    target_vector: Tuple[float, float],
) -> Tuple[Dict[int, int], Dict[int, int]]:
    target = np.asarray(target_vector, dtype=np.float32)
    pos_counts = {blob.blob_id: 0 for blob in positive_blobs}
    neg_counts = {blob.blob_id: 0 for blob in negative_blobs}
    for pos in positive_blobs:
        for neg in negative_blobs:
            ...
            if vector_error <= PAIR_VECTOR_ERROR_MAX:
                pos_counts[pos.blob_id] += 1
                neg_counts[neg.blob_id] += 1
    return pos_counts, neg_counts


def dipole_template_for_shape(
    shape: Tuple[int, int],
    pos_x: float,
    pos_y: float,
    neg_x: float,
    neg_y: float,
    sigma: float = TEMPLATE_SIGMA,
) -> np.ndarray:
    yy, xx = np.indices(shape, dtype=np.float32)
    pos_gaussian = np.exp(-(((xx - pos_x) ** 2 + (yy - pos_y) ** 2) / (2.0 * sigma * sigma)))
    neg_gaussian = np.exp(-(((xx - neg_x) ** 2 + (yy - neg_y) ** 2) / (2.0 * sigma * sigma)))
    template = pos_gaussian - neg_gaussian
...
def normalized_correlation(patch: np.ndarray, template: np.ndarray) -> float:
    patch_values = np.asarray(patch, dtype=np.float32)
    template_values = np.asarray(template, dtype=np.float32)
    patch_values = patch_values - float(np.mean(patch_values))
    template_values = template_values - float(np.mean(template_values))
    denominator = float(np.linalg.norm(patch_values) * np.linalg.norm(template_values))
    if denominator <= 1e-8:
        return 0.0
    score = float(np.sum(patch_values * template_values) / denominator)
    return float(np.clip(score, -1.0, 1.0))


def compute_template_score(z: np.ndarray, pos_x: float, pos_y: float, neg_x: float, neg_y: float) -> float:
    center_x = (pos_x + neg_x) / 2.0
    center_y = (pos_y + neg_y) / 2.0
    x0, x1, y0, y1 = crop_bounds(z.shape, center_x, center_y, TEMPLATE_PATCH_RADIUS)
    patch = z[y0:y1, x0:x1]
    if patch.size == 0:
        return 0.0
    template = dipole_template_for_shape(
        patch.shape,
        pos_x - x0,
        pos_y - y0,
        neg_x - x0,
    ...
    return normalized_correlation(patch, template)


def compute_isolation_score(z: np.ndarray, pos_x: float, pos_y: float, neg_x: float, neg_y: float) -> float:
    center_x = (pos_x + neg_x) / 2.0
    center_y = (pos_y + neg_y) / 2.0
    distance = math.hypot(neg_x - pos_x, neg_y - pos_y)
    inner_radius = distance / 2.0 + ISOLATION_RING_MARGIN
    outer_radius = inner_radius + ISOLATION_RING_WIDTH
    x0, x1, y0, y1 = crop_bounds(z.shape, center_x, center_y, int(math.ceil(outer_radius)) + 2)
    patch_abs = np.abs(z[y0:y1, x0:x1])
    if patch_abs.size == 0:
        return 0.0
    yy, xx = np.indices(patch_abs.shape, dtype=np.float32)
    local_pos_x = pos_x - x0
    local_pos_y = pos_y - y0
    local_neg_x = neg_x - x0
    local_neg_y = neg_y - y0
    local_center_x = center_x - x0
    local_center_y = center_y - y0
    pos_center_mask = (xx - local_pos_x) ** 2 + (yy - local_pos_y) ** 2 <= ISOLATION_CENTER_RADIUS ** 2
    neg_center_mask = (xx - local_neg_x) ** 2 + (yy - local_neg_y) ** 2 <= ISOLATION_CENTER_RADIUS ** 2
    center_values = []
    if np.any(pos_center_mask):
        center_values.append(float(np.max(patch_abs[pos_center_mask])))
    if np.any(neg_center_mask):
        center_values.append(float(np.max(patch_abs[neg_center_mask])))
    center_response = float(np.mean(center_values)) if center_values else 0.0
    radius_from_pair_center = np.hypot(xx - local_center_x, yy - local_center_y)
    surrounding_mask = (radius_from_pair_center >= inner_radius) & (radius_from_pair_center <= outer_radius)
    if np.any(surrounding_mask):
        surrounding_response = float(np.mean(patch_abs[surrounding_mask]))
    else:
        surrounding_response = float(np.mean(patch_abs))
    return float(center_response / (surrounding_response + 1e-6))


def classify_pair_v2(
    vector_error: float,
    amplitude_balance: float,
    pos_peak_z: float,
    neg_peak_abs_z: float,
    pos_aspect_ratio: float,
    neg_aspect_ratio: float,
) -> str:
    if (
        vector_error <= 5.0
        and amplitude_balance >= 0.35
        and pos_peak_z >= 3.5
        and neg_peak_abs_z >= 3.5
        and pos_aspect_ratio < 3.0
        and neg_aspect_ratio < 3.0
    ):
        return "high_confidence"
    if vector_error <= 8.0 and amplitude_balance >= 0.2:
...
def classify_pair_v3(
    ...
    if (
        vector_error <= 6.0
        and amplitude_balance >= 0.30
        and pos_peak_z >= 3.5
        and neg_peak_abs_z >= 3.5
        and template_score >= 0.35
        and isolation_score >= 1.5
        and pos_aspect_ratio < 3.5
        and neg_aspect_ratio < 3.5
    ):
        return "high_confidence"
    if vector_error <= 9.0 and amplitude_balance >= 0.20 and template_score >= 0.20 and isolation_score >= 1.2:
        return "medium_confidence"
    return "low_confidence"
...
def match_pairs(
    ...
    target = np.asarray(target_vector, dtype=np.float32)
    pos_partner_counts, neg_partner_counts = compute_possible_partner_counts(positive_blobs, negative_blobs, target_vector)
    ...
    for pos in positive_blobs:
        for neg in negative_blobs:
            ...
            if vector_error > PAIR_VECTOR_ERROR_MAX:
                continue
            pos_peak_z = float(pos.max_z)
            neg_peak_abs_z = float(abs(neg.min_z))
            amplitude_denominator = max(pos_peak_z, neg_peak_abs_z, 1e-6)
            amplitude_balance = min(pos_peak_z, neg_peak_abs_z) / amplitude_denominator
            area_ratio = min(pos.area, neg.area) / max(pos.area, neg.area, 1.0)
            size_mismatch_penalty = 1.0 - area_ratio
            shape_penalty = abs(pos.aspect_ratio - neg.aspect_ratio)
            elongation_penalty = max(0.0, pos.aspect_ratio - 1.0) + max(0.0, neg.aspect_ratio - 1.0)
            num_possible_partners = max(pos_partner_counts[pos.blob_id], neg_partner_counts[neg.blob_id])
            partner_ambiguity_penalty = max(0, num_possible_partners - 1)
            local_contrast_score = 0.5 * (pos_peak_z + neg_peak_abs_z) * amplitude_balance
            template_score = compute_template_score(z, float(pos.x), float(pos.y), float(neg.x), float(neg.y))
            isolation_score = compute_isolation_score(z, float(pos.x), float(pos.y), float(neg.x), float(neg.y))
            isolation_score_clipped = float(np.clip(isolation_score, 0.0, 3.0))
            confidence_v2 = classify_pair_v2(
                vector_error,
                amplitude_balance,
                pos_peak_z,
                neg_peak_abs_z,
            ...
            confidence_v3 = classify_pair_v3(
                vector_error,
                amplitude_balance,
                pos_peak_z,
                neg_peak_abs_z,
            ...
            score_v2 = (
                pos_peak_z
                + neg_peak_abs_z
                + 2.0 * amplitude_balance
                - 0.5 * vector_error
                - 0.5 * size_mismatch_penalty
                - 0.3 * partner_ambiguity_penalty
                - elongation_penalty
            )
            score_v3 = score_v2 + 4.0 * template_score + 1.5 * isolation_score_clipped - 0.5 * partner_ambiguity_penalty
            matches.append(
                PairMatch(
                    pair_id=0,
                    rank_v3=0,
                    ...
                    pos_peak_z=pos_peak_z,
                    neg_peak_abs_z=neg_peak_abs_z,
                    amplitude_balance=float(amplitude_balance),
                    area_ratio=float(area_ratio),
                    shape_penalty=float(shape_penalty),
                    pos_num_possible_partners=int(pos_partner_counts[pos.blob_id]),
                    neg_num_possible_partners=int(neg_partner_counts[neg.blob_id]),
                    num_possible_partners_pos=int(pos_partner_counts[pos.blob_id]),
                    num_possible_partners_neg=int(neg_partner_counts[neg.blob_id]),
                    num_possible_partners=int(num_possible_partners),
                    aspect_ratio_pos=float(pos.aspect_ratio),
                    aspect_ratio_neg=float(neg.aspect_ratio),
                    local_contrast_score=float(local_contrast_score),
                    template_score=float(template_score),
                    isolation_score=float(isolation_score),
                    isolation_score_clipped=float(isolation_score_clipped),
                    size_mismatch_penalty=float(size_mismatch_penalty),
                    partner_ambiguity_penalty=float(partner_ambiguity_penalty),
                    elongation_penalty=float(elongation_penalty),
                    confidence_v2=confidence_v2,
                    confidence_v3=confidence_v3,
                    score_v2=float(score_v2),
                    score_v3=float(score_v3),
                    score=float(score_v3),
                )
            )
    matches.sort(key=lambda item: item.score_v3, reverse=True)
...
def save_pair_crop(image: np.ndarray, z: np.ndarray, pair: PairMatch, path: Path) -> None:
    ...
    template = dipole_template_for_shape(z_crop.shape, pos_x, pos_y, neg_x, neg_y)
    ...
    axes[2].contour(template, levels=[0.012], colors=["red"], linewidths=0.6, alpha=0.9)
    axes[2].contour(template, levels=[-0.012], colors=["cyan"], linewidths=0.6, alpha=0.9)
    ...
    axes[2].set_title("template overlay")
    for ax in axes:
        ax.set_axis_off()
    fig.suptitle(
        "pair {0:04d} {1} score_v3={2:.2f} tmpl={3:.2f} iso={4:.2f}".format(
            pair.pair_id,
            pair.confidence_v3,
            pair.score_v3,
            pair.template_score,
            pair.isolation_score,
        ),
        fontsize=8,
    )
    fig.tight_layout()
...
def make_contact_sheet_from_crops(crop_paths: Sequence[Path], matches: Sequence[PairMatch], contact_path: Path, empty_message: str) -> None:
    ...
    if not crop_paths:
        ...
        plt.close(fig)
        return
    thumb_images = [plt.imread(str(crop_path)) for crop_path in crop_paths]
    columns = 5
    ...
    for ax, thumb, pair in zip(axes_array, thumb_images, matches):
        ax.imshow(thumb)
        ax.set_title("#{0} {1} s3={2:.1f}".format(pair.pair_id, pair.confidence_v3.replace("_confidence", ""), pair.score_v3), fontsize=8)
        ax.set_axis_off()
...
def create_crops_and_contact_sheet(
    image: np.ndarray,
    z: np.ndarray,
    ...
    crops_dir = out_dir / crop_dir_name
    ensure_dir(crops_dir)
    for old_crop in crops_dir.glob("pair_*.png"):
        old_crop.unlink()
    crop_paths: List[Path] = []
    selected_matches = list(matches[:TOP_CROP_COUNT])
    ...
    make_contact_sheet_from_crops(
        crop_paths,
        selected_matches,
        out_dir / contact_filename,
        "No matched pairs",
    )
    return crop_paths


def create_confidence_contact_sheet(
    image: np.ndarray,
    z: np.ndarray,
    matches: Sequence[PairMatch],
    out_dir: Path,
    ...
    category_matches = [match for match in matches if match.confidence_v3 == confidence_category][:TOP_CROP_COUNT]
    crop_dir_name = "crops_{0}".format(confidence_category)
    crops_dir = out_dir / crop_dir_name
    ensure_dir(crops_dir)
    for old_crop in crops_dir.glob("pair_*.png"):
        old_crop.unlink()
    crop_paths: List[Path] = []
    for pair in category_matches:
        crop_path = crops_dir / "pair_{0:04d}.png".format(pair.pair_id)
        save_pair_crop(image, z, pair, crop_path)
        crop_paths.append(crop_path)
    make_contact_sheet_from_crops(
        ...
        category_matches,
        out_dir / contact_filename,
        "No {0} pairs".format(confidence_category.replace("_", " ")),
    )
...
def write_run_summary(
    ...
    summary = {
        ...
        "thresholds": {
            ...
            "template_sigma_px": TEMPLATE_SIGMA,
            "template_patch_radius_px": TEMPLATE_PATCH_RADIUS,
            "isolation_center_radius_px": ISOLATION_CENTER_RADIUS,
            "isolation_ring_margin_px": ISOLATION_RING_MARGIN,
            "isolation_ring_width_px": ISOLATION_RING_WIDTH,
        },
        ...
        "confidence_counts": confidence_counts(matches),
        "confidence_counts_v3": confidence_counts(matches),
        "confidence_counts_v2": confidence_counts_v2(matches),
        "seed_pair_ranks": seed_pair_ranks(matches, refined_pairs),
...
def build_review_package(
    ...
    pairs_filename = "pairs_{0}_v3.csv".format(mode)
    overlay_filename = "pair_overlay_{0}_v3.png".format(mode)
    contact_filename = "contact_sheet_{0}_v3.png".format(mode)
    copy_csv(out_dir / pairs_filename, review_dir / pairs_filename)
    if mode == "manual":
        copy_csv(out_dir / "refined_seed_pairs.csv", review_dir / "refined_seed_pairs.csv")
        copy_image_as_jpg(out_dir / overlay_filename, review_dir / "pair_overlay_manual_v3.jpg")
        copy_image_as_jpg(out_dir / contact_filename, review_dir / "contact_sheet_manual_v3.jpg")
        copy_image_as_jpg(out_dir / "high_confidence_contact_sheet_v3.png", review_dir / "high_confidence_contact_sheet_manual_v3.jpg")
        copy_image_as_jpg(out_dir / "medium_confidence_contact_sheet_v3.png", review_dir / "medium_confidence_contact_sheet_manual_v3.jpg")
        crop_source = out_dir / "crops_manual_v3"
        crop_destination = review_dir / "crops_manual_v3"
        high_source = out_dir / "crops_high_confidence"
        medium_source = out_dir / "crops_medium_confidence"
    else:
        ...
        copy_image_as_jpg(out_dir / overlay_filename, review_dir / "pair_overlay_auto_v3.jpg")
        copy_image_as_jpg(out_dir / contact_filename, review_dir / "contact_sheet_auto_v3.jpg")
        copy_image_as_jpg(out_dir / "high_confidence_contact_sheet_v3.png", review_dir / "high_confidence_contact_sheet_auto_v3.jpg")
        copy_image_as_jpg(out_dir / "medium_confidence_contact_sheet_v3.png", review_dir / "medium_confidence_contact_sheet_auto_v3.jpg")
        crop_source = out_dir / "crops_auto_v3"
        crop_destination = review_dir / "crops_auto_v3"
        high_source = out_dir / "crops_high_confidence"
        medium_source = out_dir / "crops_medium_confidence"
    ...
    for old_crop in crop_destination.glob("*.jpg"):
        old_crop.unlink()
    for crop in sorted(crop_source.glob("pair_*.png"))[:TOP_CROP_COUNT]:
        copy_image_as_jpg(crop, crop_destination / crop.with_suffix(".jpg").name)
    if high_source is not None:
        high_destination = review_dir / "crops_high_confidence"
        ensure_dir(high_destination)
        for old_crop in high_destination.glob("*.jpg"):
            old_crop.unlink()
        for crop in sorted(high_source.glob("pair_*.png"))[:TOP_CROP_COUNT]:
            copy_image_as_jpg(crop, high_destination / crop.with_suffix(".jpg").name)
    if medium_source is not None:
        medium_destination = review_dir / "crops_medium_confidence"
        ensure_dir(medium_destination)
        for old_crop in medium_destination.glob("*.jpg"):
            old_crop.unlink()
        for crop in sorted(medium_source.glob("pair_*.png"))[:TOP_CROP_COUNT]:
            copy_image_as_jpg(crop, medium_destination / crop.with_suffix(".jpg").name)
...
def category_summary_text(summary: Optional[Dict]) -> str:
    if not summary:
        return "not available"
    counts = summary.get("confidence_counts_v3") or summary.get("confidence_counts", {})
    return "high={0}, medium={1}, low={2}".format(
        counts.get("high_confidence", 0),
        counts.get("medium_confidence", 0),
        counts.get("low_confidence", 0),
    )


def vector_summary_text(summary: Optional[Dict]) -> str:
    if not summary or not summary.get("target_vector"):
        return "not available"
    vector = summary["target_vector"]
    return "({0:.3f}, {1:.3f})".format(float(vector["dx"]), float(vector["dy"]))


def confidence_high_count(summary: Optional[Dict], key: str) -> str:
    if not summary:
        return "n/a"
    counts = summary.get(key, {})
    return str(counts.get("high_confidence", "n/a"))


def seed_rank_lines(summary: Optional[Dict]) -> List[str]:
    if not summary or not summary.get("seed_pair_ranks"):
        return ["- Not available until manual mode is run."]
    lines = []
    for item in summary["seed_pair_ranks"]:
        lines.append(
            "- Seed pair {0}: rank `{1}`, near top 10: `{2}`, center error `{3:.3f}`".format(
                item.get("seed_pair_id"),
                item.get("rank"),
                item.get("kept_near_top10"),
                float(item.get("center_match_error") or 0.0),
            )
        )
    return lines
...
def update_review_readme(review_dir: Path) -> None:
    ...
    v2_manual = load_json_if_exists(Path("docs/review_runs/dipole_pair_detector_v2/summary_manual.json"))
    v2_auto = load_json_if_exists(Path("docs/review_runs/dipole_pair_detector_v2/summary_auto.json"))
    positive_count = csv_count(review_dir / "positive_blobs.csv")
    negative_count = csv_count(review_dir / "negative_blobs.csv")
    manual_mode_conclusion = "not available"
    if manual and auto:
        manual_vector = manual.get("target_vector", {})
        auto_vector = auto.get("target_vector", {})
        vector_gap = math.hypot(
            float(manual_vector.get("dx", 0.0)) - float(auto_vector.get("dx", 0.0)),
            float(manual_vector.get("dy", 0.0)) - float(auto_vector.get("dy", 0.0)),
        )
        manual_mode_conclusion = "yes" if vector_gap <= 2.0 else "uncertain"
    ...
    lines = [
        ...
        "| --- | ---: | ---: |",
        "| Target vector dx,dy | {0} | {1} |".format(vector_summary_text(manual), vector_summary_text(auto)),
        "| Matched count | {0} | {1} |".format(
            manual.get("matched_pair_count", "n/a") if manual else "n/a",
            auto.get("matched_pair_count", "n/a") if auto else "n/a",
        ),
        "| V3 confidence counts | {0} | {1} |".format(category_summary_text(manual), category_summary_text(auto)),
        "| V2 high confidence in same rows | {0} | {1} |".format(
            confidence_high_count(manual, "confidence_counts_v2"),
            confidence_high_count(auto, "confidence_counts_v2"),
        ),
        "| V3 high confidence | {0} | {1} |".format(
            confidence_high_count(manual, "confidence_counts_v3"),
            confidence_high_count(auto, "confidence_counts_v3"),
        ),
        "",
        "## V2 vs V3 High-Confidence Change",
        "",
        "- Previous v2 manual high-confidence count: `{0}`".format(confidence_high_count(v2_manual, "confidence_counts")),
        "- Current v3 manual high-confidence count: `{0}`".format(confidence_high_count(manual, "confidence_counts_v3")),
        "- Previous v2 auto high-confidence count: `{0}`".format(confidence_high_count(v2_auto, "confidence_counts")),
        "- Current v3 auto high-confidence count: `{0}`".format(confidence_high_count(auto, "confidence_counts_v3")),
        "",
        "## Seed Pair Rank Check",
        "",
    ]
    lines.extend(seed_rank_lines(manual))
    lines.extend(
        [
            "",
            "## Conclusion",
            "",
            "- Manual seed is still the main mode: `{0}`".format(manual_mode_conclusion),
            "- Positive blobs: `{0}`".format(positive_count if positive_count is not None else "not available"),
            "- Negative blobs: `{0}`".format(negative_count if negative_count is not None else "not available"),
            "",
            "## V3 Confidence Rules",
            "",
...
def confidence_counts(matches: Sequence[PairMatch]) -> Dict[str, int]:
    counts = {"high_confidence": 0, "medium_confidence": 0, "low_confidence": 0}
    for match in matches:
        counts[match.confidence_v3] = counts.get(match.confidence_v3, 0) + 1
    return counts


def confidence_counts_v2(matches: Sequence[PairMatch]) -> Dict[str, int]:
    counts = {"high_confidence": 0, "medium_confidence": 0, "low_confidence": 0}
    for match in matches:
        counts[match.confidence_v2] = counts.get(match.confidence_v2, 0) + 1
    return counts


def seed_pair_ranks(matches: Sequence[PairMatch], refined_pairs: Sequence[RefinedSeedPair], tolerance_px: float = 5.0) -> List[Dict]:
    ranks: List[Dict] = []
    for seed in refined_pairs:
        best_rank = None
        best_error = None
        seed_points = [(seed.refined1_x, seed.refined1_y), (seed.refined2_x, seed.refined2_y)]
        for match in matches:
            match_points = [(match.pos_x, match.pos_y), (match.neg_x, match.neg_y)]
            direct_error = math.hypot(seed_points[0][0] - match_points[0][0], seed_points[0][1] - match_points[0][1]) + math.hypot(seed_points[1][0] - match_points[1][0], seed_points[1][1] - match_points[1][1])
            swapped_error = math.hypot(seed_points[0][0] - match_points[1][0], seed_points[0][1] - match_points[1][1]) + math.hypot(seed_points[1][0] - match_points[0][0], seed_points[1][1] - match_points[0][1])
            error = min(direct_error, swapped_error)
            if best_error is None or error < best_error:
                best_error = error
                best_rank = match.pair_id
        ranks.append(
            {
                "seed_pair_id": seed.seed_pair_id,
                "rank": best_rank if best_error is not None and best_error <= tolerance_px * 2 else None,
                "center_match_error": best_error,
                "kept_near_top10": bool(best_rank is not None and best_rank <= 10 and best_error is not None and best_error <= tolerance_px * 2),
            }
        )
    return ranks
...
def run_detector(args: argparse.Namespace) -> Dict:
    ...
    matches = match_pairs(z, positive_blobs, negative_blobs, target_vector)
    pairs_filename = "pairs_{0}_v3.csv".format(mode)
    overlay_filename = "pair_overlay_{0}_v3.png".format(mode)
    contact_filename = "contact_sheet_{0}_v3.png".format(mode)
    write_csv(out_dir / pairs_filename, matches, pair_fieldnames())
    write_csv(out_dir / "pairs.csv", matches, pair_fieldnames())
    plot_pair_overlay(z, matches, target_vector, out_dir / overlay_filename, "{0} v3".format(mode))
    plot_pair_overlay(z, matches, target_vector, out_dir / "pair_overlay.png", "{0} v3".format(mode))
    create_crops_and_contact_sheet(image, z, matches, out_dir, contact_filename, "crops_{0}_v3".format(mode))
    create_confidence_contact_sheet(image, z, matches, out_dir, "high_confidence", "high_confidence_contact_sheet_v3.png")
    create_confidence_contact_sheet(image, z, matches, out_dir, "medium_confidence", "medium_confidence_contact_sheet_v3.png")
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Dipole Pair Detector v3")
